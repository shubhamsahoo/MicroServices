jQuery(document).ready(function ($) {
	...
	$("#regForm").submit(function () {
		...
		let mobNo = this.mobileNo.value;
		let email = this.email.value;
		let password = this.password.value;
		let cfmPassword = this.confirmPassword.value;
		if (firstName == null || !firstName.trim()) {
			alert(firstName);
			$('#firstName').css('border-bottom', '.0625rem solid red');
			return false;
		}
		if (lastName == null || !lastName.trim()) {
			alert(firstName);
			$('#lastName').css('border-bottom', '.0625rem solid red');
			return false;
		}
		if (dob.getFullYear() >= (date.getFullYear()-5)) {
			$('#dob').css('border-bottom', '.0625rem solid red');
			return false;
		}
		return true;
	});
	
	$('#reg_pass, #cfm_pass').on('keyup', function () {
		if ($('#reg_pass').val() == $('#cfm_pass').val()) {
			$('#cfmPassword').css('color', '#088dcd');
			$(':input[type="submit"]').prop('disabled', false);
			//$('#cfmPassword').css('color', '#b7b7b7');
		} else{
			$(':input[type="submit"]').prop('disabled', true);
			$('#cfmPassword').css('color', 'red');
		}
